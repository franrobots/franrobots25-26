...
// bool find_blue() {
//   uint16_t C9 = sensor_values[0];
//   uint16_t R = sensor_values[1];
//   uint16_t G = sensor_values[2];
//   uint16_t B = sensor_values[3];

//   if (C9 < 260) return false;
//   if (R > 10) return false;
//   float gb = (float)G / (float)(B + 1);
//   bool ratio_ok = (gb > 1.9f && gb < 2.6f);
//   bool range_ok = (G >= 80 && G <= 90) && (B >= 35 && B <= 45);
//   bool color_ok = (B > 30 && R < 10 && G > 70);

//   return (ratio_ok && range_ok && color_ok);
// }

bool find_blue() {
  return (sensor_values[0] > 456 && sensor_values[0] < 540 && sensor_values[3] > 84 && sensor_values[3] < 162 && abs(gyro.getInclinationAngle()) < 5) ? true : false; // Robocup
  // return (sensor_values[0] > 321 && sensor_values[0] < 377 && sensor_values[3] > 53 && sensor_values[3] < 97 && abs(gyro.getInclinationAngle()) < 5) ? true : false; // Robocup
  // return (sensor_values[0] > 211 && sensor_values[0] < 271 && sensor_values[3] > 0 && sensor_values[3] < 50 && abs(gyro.getInclinationAngle()) < 5) ? true : false; // xoxo capengo
}

bool find_black() { // First parameter calibrated w black tape and second was calibrated w black sticker
  // return (sensor_values[0] > 32 && sensor_values[0] < 96 && sensor_values[3] > 31 && sensor_values[3] < 114 && abs(gyro.getInclinationAngle()) < 5) ? true : false; // Hotel
  return (sensor_values[0] > 0 && sensor_values[0] < 30 && sensor_values[3] > 30 && sensor_values[3] < 60 && abs(gyro.getInclinationAngle()) < 5) ? true : false; // funciona V2
...
bool find_grey() { // First parameter calibrated w gray tape and second was calibrated w gray sticker 
  ...
  // return (sensor_values[0] > 903 && sensor_values[0] < 1043 && sensor_values[3] > 411 && sensor_values[3] < 505 && abs(gyro.getInclinationAngle()) < 5) ? true : false;
  return (sensor_values[0] > 600 && sensor_values[0] < 1043 && sensor_values[3] > 411 && sensor_values[3] < 485 && abs(gyro.getInclinationAngle()) < 5) ? true : false;
...
bool release_kits(uint8_t reps, bool sideFlag) {
 if (verifyVictms(sideFlag)) {return true;}
