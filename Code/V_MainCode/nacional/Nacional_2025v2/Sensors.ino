...
bool find_blue() {
  return (sensor_values[0] > 359 && sensor_values[0] < 419 && sensor_values[3] > 50 && sensor_values[3] < 110 && abs(gyro.getInclinationAngle()) < 5) ? true : false; //
  // return (sensor_values[0] > 0 && sensor_values[0] < 120 && sensor_values[3] > 180 && sensor_values[3] < 290 && abs(gyro.getInclinationAngle()) < 5) ? true : false; //
}
bool find_black() { // First parameter calibrated w black tape and second was calibrated w black sticker 
  return (sensor_values[0] > 0 && sensor_values[0] < 80 && sensor_values[3] > 0 && sensor_values[3] < 80 && abs(gyro.getInclinationAngle()) < 5) ? true : false;
  // return (sensor_values[0] > 23 && sensor_values[0] < 83 && sensor_values[3] > 7 && sensor_values[3] < 68 && abs(gyro.getInclinationAngle()) < 5) ? true : false;
...
bool find_grey() { // First parameter calibrated w gray tape and second was calibrated w gray sticker 
  ...
  return (sensor_values[0] > 374 && sensor_values[0] < 424 && sensor_values[3] > 78 && sensor_values[3] < 138 && abs(gyro.getInclinationAngle()) < 5) ? true : false;
}
