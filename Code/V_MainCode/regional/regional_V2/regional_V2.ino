...
public:
  ...
  int16_t convertGyro(int16_t value, int16_t offset) {
    value = value - offset;
    return (value / abs(value)) * ((abs(value) > 180) ? abs(value) - 360 : abs(value));
  }
...
float Encoder() {
  ...
  portEXIT_CRITICAL(&mux);
   if (cmValue){
     return cnt * 0.00590f;  // 0.00590 roda menor // 0.00630 roda grande 585 // Change the value if have some ramps 
   }else{
    return cnt * 0.00590f;  // 0.00590 roda grande
  }
}
...
int dist_sensors_offsets[8] = {23, 30,  51,  62,  58, 51,  118,  30};
uint16_t sensors_target_value[] = {40, 340, 640, 940};
...
void taskOnCore0(void *pvParameters) {
  ...
  unsigned long timeLimit = 7 * 60 * 1000;  // 8 minutos
  for (;;) {
    if (!bnoFlag) {
