...
#define KIT_CENTER 83
#define KIT_LEFT 54
#define KIT_RIGHT 106
...
//                                        0   1   2   3   4   5   6    7
constexpr int dist_sensors_offsets[8] = {36, 30, 50, 58, 55, 42, 116, 66}; // positivo = aumenta; negativo = subtrai.
// constexpr int dist_sensors_offsets[8] = {35, 30, 50, 58, 63, 42, 115, 67}; // positivo = aumenta; negativo = subtrai.
// constexpr int dist_sensors_offsets[8] = {20, 35, 50, 58, 62, 42, 115, 72}; // positivo = aumenta; negativo = subtrai.
...
void setup() {
  analogReadResolution(10); // Resolução de 10 bits dos LDR's
...
void taskOnCore0(void *pvParameters) {
  ...
  unsigned long timeLimit = 7 * 60 * 1000;  // 7 minutos
  for (;;) {
