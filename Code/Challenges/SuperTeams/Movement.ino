...
void moveTile()
  {
    ...
    if (no){
      if (getColor() == "blue"){
        ...
        vTaskDelay(pdMS_TO_TICKS(500)); // Centro do ladrilho
        stopTank();
        //blink_led(5, 3, true);
  ...
  if (walkByEncoder(28, walkByCoordinate)) {
  ...
  int incAngle = -gyro.getInclinationAngle();
    if (abs(incAngle) > 10) {
      ...
      if (cont >= 2) {
        ...
        addcm = constrain(addcm + delta, -7, 9);  // limita entre -5 e +5 cm
        cont = 0;
      }
    } else {
      addcm = 0;
...
void return2init() {
  ...
  for (uint8_t i = 0; i < teste.len(); i++) {
    ...
    walkByEncoder(28, true); // Mesmo valor do encoder de cima, de def 
  }
  stopTank();
  blink_led(10, 4, true);
